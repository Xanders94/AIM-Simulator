...
public class Voronoi {
    ...
    public Map<Vertex, Path2D> generateBorderPolygons(Map<Vertex, Path2D> polygons){
    	LinkedList<GraphEdge> borders = new LinkedList<GraphEdge>();
    	
    	List<Site> left = new ArrayList<Site>();
    	List<Site> right = new ArrayList<Site>();
    	List<Site> up = new ArrayList<Site>();
    	List<Site> down = new ArrayList<Site>();
    	
    	//add existing sites to the 
    	for(Site site : this.sites){
    		if(site.isBogus){
    			continue;
    		}
    		if(site.xPos == this.minX){
    			left.add(site);
    		}
    		if(site.xPos == this.maxX){
    			right.add(site);
    		}
    		if(site.yPos == this.minY){
    			down.add(site);
    		}
    		if(site.yPos == this.maxY){
    			up.add(site);
    		}
    	}
    	//generate lines and assign to the closest site to the mid point of new line
    	//add to linked list
    	//left
    	borders.addAll(processBorder(left,this.allEdges,true,true));
    	//right
    	borders.addAll(processBorder(right,this.allEdges,true,false));
    	//up
    	borders.addAll(processBorder(up,this.allEdges,false,false));
    	//down
    	borders.addAll(processBorder(down,this.allEdges,false,true));
    	//add allEdges to working set
    	borders.addAll(this.allEdges);
    	//detect edge sites
    	
    	//start test
    	/*this.allEdges.addAll(borders);
    	int i = 0;
    	if(i == 0){
    		return null;
    	}*/
    	...
    	ArrayList<GraphEdge> edgeLines = new ArrayList<GraphEdge>();
    	ArrayList<Integer> edgeSites = new ArrayList<Integer>();
    	
    	for(GraphEdge edge : borders){
    		if(edge.site1 == -1){
				edgeLines.add(edge);
				if(!edgeSites.contains(edge.site2)){
					edgeSites.add(edge.site2);
				}
    		} else if(edge.site2 == -1){
    			edgeLines.add(edge);
    			if(!edgeSites.contains(edge.site1)){
    				edgeSites.add(edge.site1);
    			}
    		}
    	}
    	//if no edges detected, return null
    	if(edgeLines.isEmpty()){
    		return null;
    	}
    	//for each siteId organize the lines making the polygon and generate a shape
    	HashMap<Vertex, Path2D> edgePolys = new HashMap<Vertex, Path2D>(polygons);
    	LinkedList<GraphEdge> polygon;
    	for(Site site : this.sites){
    		if(edgeSites.contains(site.sitenbr)){
    			polygon = filterBySite(borders,site.sitenbr);
    			if(polygon != null){
    				edgePolys.put(new Vertex(site.xPos, site.yPos), generatePolygon(polygon));
    			}
    		}
    	}
    	return edgePolys;
    ...
    /**
     * generates a polygon from graph edges
     * @param polygon
     * @return
     */
    private Path2D generatePolygon(LinkedList<GraphEdge> polygon) {
    	Path2D result = new Path2D.Double();
    	GraphEdge start = polygon.get(0);

    	//determine the start of the next line
    	if(Math.abs(start.xPos1 - polygon.get(1).xPos1) <= this.epsilon && Math.abs(start.yPos1 - polygon.get(1).yPos1) <= this.epsilon){
    		result.moveTo(start.xPos2, start.yPos2);
    		result.lineTo(start.xPos1, start.yPos1);
    		
    	} else if(Math.abs(start.xPos1 - polygon.get(1).xPos2) <= this.epsilon && Math.abs(start.yPos1 - polygon.get(1).yPos2) <= this.epsilon){
    		result.moveTo(start.xPos2, start.yPos2);
    		result.lineTo(start.xPos1, start.yPos1);
    		
    	} else if(Math.abs(start.xPos2 - polygon.get(1).xPos1) <= this.epsilon && Math.abs(start.yPos2 - polygon.get(1).yPos1) <= this.epsilon){
    		result.moveTo(start.xPos1, start.yPos1);
    		result.lineTo(start.xPos2, start.yPos2);
    		
    	} else if(Math.abs(start.xPos2 - polygon.get(1).xPos2) <= this.epsilon && Math.abs(start.yPos2 - polygon.get(1).yPos2) <= this.epsilon){
    		result.moveTo(start.xPos1, start.yPos1);
    		result.lineTo(start.xPos2, start.yPos2);
    	} else {
    		System.out.println("crash about to happen");
    	}
		for(int i = 1; i < polygon.size(); i++){
			if(i == (polygon.size() - 1)){
				drawConnection(result, polygon.get(i), start);
			} else {
				drawConnection(result, polygon.get(i), polygon.get(i+1));
			}
		}
		result.closePath();
		return result;
    ...
    /**
     * @Author Alexander Humphry
     * draws a line on a GraphEdge "edge" assuming the cursor is not on the point of connection with the next line
     * @param input
     * @param edge
     * @param edge2
     * @return
     */
	private void drawConnection(Path2D input,GraphEdge edge, GraphEdge edge2) {
		
		if(edge.xPos1 == edge2.xPos1 && edge.yPos1 == edge2.yPos1){
    		input.lineTo(edge.xPos1, edge.yPos1);
    		
    	} else if(edge.xPos1 == edge2.xPos2 && edge.yPos1 == edge2.yPos2){
    		input.lineTo(edge.xPos1, edge.yPos1);
    		
    	} else if(edge.xPos2 == edge2.xPos1 && edge.yPos2 == edge2.yPos1){
    		input.lineTo(edge.xPos2, edge.yPos2);
    		
    	} else if(edge.xPos2 == edge2.xPos2 && edge.yPos2 == edge2.yPos2){
    		input.lineTo(edge.xPos2, edge.yPos2);
    	}
	}
     ...
     * @return
     */
    private LinkedList<GraphEdge> filterBySite(LinkedList<GraphEdge> edges, int siteId){
    	
    	ArrayList<GraphEdge> inter = new ArrayList<GraphEdge>();
    	LinkedList<GraphEdge> result = new LinkedList<GraphEdge>();
    	
    	GraphEdge start;
    	GraphEdge current;

    	for(GraphEdge edge : edges){
    		if(edge.site1 == siteId || edge.site2 == siteId){
    			inter.add(edge);
    		}
    	}
    	if(inter.isEmpty()){
    	...
    	int debugCount = 0;
    	while(inter.size() > 0){
    		for(GraphEdge edge : inter){
    			//if line connects, add to result
	    		if((Math.abs(edge.xPos1 - current.xPos1) <= this.epsilon && Math.abs(edge.yPos1 - current.yPos1) <= this.epsilon) ||
	    				(Math.abs(edge.xPos2 - current.xPos2) <= this.epsilon && Math.abs(edge.yPos2 - current.yPos2) <= this.epsilon) || 
	    				(Math.abs(edge.xPos2 - current.xPos1) <= this.epsilon && Math.abs(edge.yPos2 - current.yPos1) <= this.epsilon) || 
	    				(Math.abs(edge.xPos1 - current.xPos2) <= this.epsilon && Math.abs(edge.yPos1 - current.yPos2) <= this.epsilon)){
	    			result.add(edge);
	    			current = edge;
	        		debugCount = 0;
	    			break;
     ...
     * @param min ,true if the fixed x or y value is the lowest of the 2 possible borders
     * @return list of border edges
     */
    private LinkedList<GraphEdge> processBorder(List<Site> left,Boolean vertical, Boolean min){
    	LinkedList<GraphEdge> temp = new LinkedList<GraphEdge>();
    	GraphEdge tempEdge = null;
    	//old generation mechanism
    	if(vertical){
    		for(int i = 0; i < left.size(); i++){
    			...
    			if(i == 0){
    				if(min){
						tempEdge.site1 = -1;
						tempEdge.site2 = left.get(i).sitenbr;
						tempEdge.xPos1 = this.minX;
	    				tempEdge.yPos1 = this.minY;	
					} else {
						tempEdge.site1 = left.get(i).sitenbr;
						tempEdge.site2 = -1;
						tempEdge.xPos1 = this.maxX;
	    				tempEdge.yPos1 = this.minY;	
					}
    			} else {
    				if(min){
    				...
    				tempEdge.xPos1 = left.get(i - 1).xPos;
    				tempEdge.yPos1 = (left.get(i - 1).yPos + left.get(i).yPos) / 2;
    			}
    			//for last point
    			if(i == left.size() - 1){
    				if(min){
	    				tempEdge.xPos2 = this.minX;
	    				tempEdge.yPos2 = this.maxY;
    				} else {
    					tempEdge.xPos2 = this.maxX;
	    				tempEdge.yPos2 = this.maxY;
    				}
    			} else {
    				tempEdge.xPos2 = left.get(i).xPos;
    				tempEdge.yPos2 = (left.get(i).yPos + left.get(i + 1).yPos) / 2;
    			}
    			temp.add(tempEdge);
    		}
    	} else {
    		for(int i = 0; i < left.size(); i++){
    			...
    			if(i == 0){
    				...
    				tempEdge.xPos1 = (left.get(i - 1).xPos  + left.get(i).xPos) / 2;
    				tempEdge.yPos1 = left.get(i - 1).yPos;
    			}
    			//for last point
    			if(i == left.size() - 1){
    				if(min){
    				...
    				tempEdge.xPos2 = (left.get(i).xPos + left.get(i + 1).xPos) / 2;
    				tempEdge.yPos2 = left.get(i).yPos;
    			}
    			temp.add(tempEdge);
    		}
    	}
    ...
    /**
     * generates a set of graph edges along the border of the graph based on the edges terminating at the borders
     * 
     * @param sites
     * @param edges
     * @param vertical
     * @param min
     * @return an ordered linked list of graph edges
     */
    private LinkedList<GraphEdge> processBorder(List<Site> sites, List<GraphEdge> edges,Boolean vertical, Boolean min){
    	LinkedList<GraphEdge> temp = new LinkedList<GraphEdge>();
    	GraphEdge tempEdge = null;
    	...
    	if(vertical){
    		if(min){
    			//if the end of a line is close to the border, attach a point to the border
    			for(GraphEdge edge : edges){
    				if(Math.abs(this.minX - edge.xPos1) <= this.epsilon){
    					points.add(new Vertex(this.minX, edge.yPos1));
    				} else if(Math.abs(this.minX - edge.xPos2) <= this.epsilon){
    					points.add(new Vertex(this.minX, edge.yPos2));
    				}
    			}
    		} else {
    			for(GraphEdge edge : edges){
    				if(Math.abs(this.maxX - edge.xPos1) <= this.epsilon){
    					points.add(new Vertex(this.maxX, edge.yPos1));
    				} else if(Math.abs(this.maxX - edge.xPos2) <= this.epsilon){
    					points.add(new Vertex(this.maxX, edge.yPos2));
    				}
    			}
    		}
    	} else {
    		if(min){
    			//if the end of a line is close to the border, attach a point to the border
    			for(GraphEdge edge : edges){
    				if(Math.abs(this.minY - edge.yPos1) <= this.epsilon){
    					points.add(new Vertex(edge.xPos1, this.minY));
    				} else if(Math.abs(this.minY - edge.xPos2) <= this.epsilon){
    					points.add(new Vertex(edge.xPos2, this.minY));
    				}
    			}
    		} else {
    			for(GraphEdge edge : edges){
    				if(Math.abs(this.maxY - edge.yPos1) <= this.epsilon){
    					points.add(new Vertex(edge.xPos1, this.maxY));
    				} else if(Math.abs(this.maxY - edge.yPos2) <= this.epsilon){
    					points.add(new Vertex(edge.xPos2, this.maxY));
    				}
    			}
    		}
    	}
    	...
    	if(vertical){
    		if(min){
    			points.add(new Vertex(this.minX,this.minY));
    			points.add(new Vertex(this.minX,this.maxY));
    		} else {
    			points.add(new Vertex(this.maxX,this.minY));
    			points.add(new Vertex(this.maxX,this.maxY));
    		}
    	} else {
    		if(min){
    			points.add(new Vertex(this.minX,this.minY));
    	...
    	//remove duplicate points and convert to ArrayList
    	if(vertical){
    		Collections.sort(points, new Comparator<Vertex> (){
    			public int compare(Vertex arg1, Vertex arg2){
        			Double result = (arg1.yPos - arg2.yPos);
    		...
    		Collections.sort(points, new Comparator<Vertex> (){
    			public int compare(Vertex arg1, Vertex arg2){
        			Double result = (arg1.xPos - arg2.xPos);
        			return result.intValue();
        		}
        	});
    	}
    	...
    	while(modified){
    		...
    		for(int i = 1; i < points.size(); i++){
    			if(points.get(i - 1).xPos == points.get(i).xPos && points.get(i - 1).yPos == points.get(i).yPos){
    				points.remove(i);
    				modified = true;
    				break;
    			}
    	...
    	for(int i = 1; i < points.size(); i++){
    		tempEdge = new GraphEdge();
    		tempEdge.xPos1 = points.get(i - 1).xPos;
    		tempEdge.yPos1 = points.get(i - 1).yPos;
    		tempEdge.xPos2 = points.get(i).xPos;
    		tempEdge.yPos2 = points.get(i).yPos;
    		determineEdgeSites(sites, tempEdge, vertical, min);
    		temp.add(tempEdge);
    	}
    	return temp;
    }
     ...
     * @author Alexander Humphry
     * 
     * determines the closest sites to an edge on the border of a graph
     * 
     * @param sites2
     * @param tempEdge
     * @param vertical
     * @param min
     */
    private void determineEdgeSites(List<Site> sites, GraphEdge tempEdge, boolean vertical, boolean min) {
		if(min){
			tempEdge.site1 = -1;
			tempEdge.site2 = closestSite(sites, new Vertex((tempEdge.xPos1 + tempEdge.xPos2) / 2, (tempEdge.yPos1 + tempEdge.yPos2) / 2));
		} else{
			tempEdge.site1 = closestSite(sites, new Vertex((tempEdge.xPos1 + tempEdge.xPos2) / 2, (tempEdge.yPos1 + tempEdge.yPos2) / 2));
			tempEdge.site2 = -1;
		}
	}
    /**
     * @author Alexander Humphry
     * determines the closest site to a vertex
     * @param sites
     * @param vertex
     * @return the id of the closest site
     */
	private int closestSite(List<Site> sites, Vertex vertex) {
		double closestSiteDist = -1.0;
		double tempDist;
		int closestSiteId = -1;
		for(Site site : sites){
			tempDist = Math.sqrt((site.xPos - vertex.xPos)*(site.xPos - vertex.xPos) + (site.yPos - vertex.yPos)*(site.yPos - vertex.yPos));
			if(closestSiteDist < 0 || closestSiteDist > tempDist){
				closestSiteDist = tempDist;
				closestSiteId = site.sitenbr;
			}
		}
